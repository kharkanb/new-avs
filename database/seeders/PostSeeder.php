...
class PostSeeder extends Seeder
{
    ...
    {
        $this->command->info('🚀 شروع ایجاد پست‌ها و فیدرها...');
        ...
        foreach ($postsData as $item) {
            // بررسی وجود پست
            $existingPost = DB::table('posts')->where('name', $item['post'])->first();
            
            if ($existingPost) {
                $postId = $existingPost->id;
                $this->command->info("⏩ پست {$item['post']} از قبل وجود داشت");
            } else {
                ...
                $this->command->info("✅ پست {$item['post']} ایجاد شد");
                $totalPosts++;
            }
            
            // ایجاد فیدرها
            $feederCount = 0;
            foreach ($item['feeders'] as $feederName) {
                $existingFeeder = DB::table('feeders')
                    ->where('post_id', $postId)
                    ->where('name', $feederName)
                    ->first();
            ...
            if ($feederCount > 0) {
                $this->command->info("   📌 {$feederCount} فیدر جدید برای {$item['post']} اضافه شد");
            }
        }

        $this->command->info("\n📊 نتیجه نهایی:");
        $this->command->info("   ✅ {$totalPosts} پست جدید ایجاد شد");
        $this->command->info("   ✅ {$totalFeeders} فیدر جدید ایجاد شد");
        $this->command->info("   📌 کل پست‌ها: " . DB::table('posts')->count());
        $this->command->info("   📌 کل فیدرها: " . DB::table('feeders')->count());
        $this->command->info("\n✅ همه پست‌ها و فیدرها با موفقیت ایجاد شدند!");
    }
}